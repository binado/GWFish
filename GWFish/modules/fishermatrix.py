...
def fft_derivs_at_detectors(deriv_list, frequency_vector):
    ...
    for deriv in deriv_list:
        ffd_deriv_list.append(
            fft.fft_lal_timeseries(deriv, delta_f, f_start=0.0).data.data
        )
...
class Derivative:
    ...
    def projection_at_parameters(self):
        if self._projection_at_parameters is None:
            self._projection_at_parameters = det.projection(
                self.local_params,
                self.detector,
    ...
    def with_respect_to(self, target_parameter):
        ...
        if target_parameter == "luminosity_distance":
            derivative = (
                -1.0
                / self.local_params[target_parameter]
                * self.projection_at_parameters
            )
            ...
            self.pv_set1[target_parameter] = pv - dp / 2.0
            self.pv_set2[target_parameter] = pv + dp / 2.0

            if target_parameter in [
                "ra",
                "dec",
                "psi",
            ]:  # these parameters do not influence the waveform
                signal1 = det.projection(
                    self.pv_set1,
                    self.detector,
                    self.waveform_at_parameters[0],
                    self.waveform_at_parameters[1],
                )
                signal2 = det.projection(
                    self.pv_set2,
                    self.detector,
                    self.waveform_at_parameters[0],
                    self.waveform_at_parameters[1],
                ...
                self.pv_set1["geocent_time"] = 0.0
                self.pv_set2["geocent_time"] = 0.0

                waveform_obj1 = self.waveform_class(
                    self.waveform, self.pv_set1, self.data_params
                )
                wave1 = waveform_obj1()
                t_of_f1 = waveform_obj1.t_of_f

                waveform_obj2 = self.waveform_class(
                    self.waveform, self.pv_set2, self.data_params
                )
                wave2 = waveform_obj2()
                ...
                signal1 = det.projection(
                    self.pv_set1, self.detector, wave1, t_of_f1 + self.tc
                )
                signal2 = det.projection(
                    self.pv_set2, self.detector, wave2, t_of_f2 + self.tc
                )

                derivative = (
                    np.exp(2j * np.pi * self.detector.frequencyvector * self.tc)
                    * (signal2 - signal1)
                    / dp
                )
...
def sky_localization_percentile_factor(percentile: float = 90.0) -> float:
    ...
    return -2 * np.log(1 - percentile / 100.0) * (180 / np.pi) ** 2
...
def compute_detector_fisher(
    ...
    if redefine_tf_vectors:
        signal, timevector, frequencyvector = det.projection(
    ...
    component_SNRs = det.SNR(
        detector, signal, use_duty_cycle, frequencyvector=frequencyvector
    )
...
def compute_network_errors(
    ...
    for k in tqdm(range(n_signals)):
        ...
        network_snr_square = 0.0

        signal_parameter_values = parameter_values.iloc[k]

        for detector in network.detectors:
            detector_fisher, detector_snr_square = compute_detector_fisher(
                detector,
                signal_parameter_values,
                fisher_parameters,
                f_ref,
    ...
    if save_matrices:
        ...
        np.save(
            save_matrices_path / f"fisher_matrices{matrix_naming_postfix}.npy",
            fisher_matrices,
        )
        np.save(
            save_matrices_path / f"inv_fisher_matrices{matrix_naming_postfix}.npy",
            inv_fisher_matrices,
        )
...
def errors_file_name(
    ...
    return f"Errors_{sub_network}_{population_name}_SNR{network.detection_SNR[1]:.0f}"
...
def output_to_txt_file(
    ...
    row_format = "%s " + " ".join(
        [decimal_output_format for _ in range(save_data.shape[1] - 1)]
    )
